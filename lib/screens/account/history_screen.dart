...
class _HistoryScreenState extends State<HistoryScreen> {
  late Future<List<KitchenModel>> _kitchensFuture;
  ...
  void _loadData() {
    _kitchensFuture = context.read<StorageService>().loadKitchens();
  }
  ...
  List<KitchenModel> _sort(List<KitchenModel> list) {
    final sorted = [...list];
    switch (_sortOption) {
      case _SortOption.newest:
  ...
  // ── Delete selected ──────────────────────────────────────
  Future<void> _deleteSelected(List<KitchenModel> all) async {
    ...
    await storage.saveKitchens(remaining);
    _reload();
  }
            ...
            ..._SortOption.values.map((opt) {
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: selected
                        ? AppTheme.burntOrange.withValues(alpha: 0.2)
                  ...
                  child: Row(
                    children: [
                      Icon(opt.icon,
                          color: selected ? AppTheme.goldenYellow : Colors.white54,
                          size: 20),
                      const SizedBox(width: 12),
                      Text(opt.label,
                          style: TextStyle(
                            color: selected ? AppTheme.goldenYellow : Colors.white70,
                            fontWeight: selected
                                ? FontWeight.w700
                                : FontWeight.w400,
                            fontSize: 15,
                      ...
                      if (selected)
                        const Icon(Icons.check_rounded,
                            color: AppTheme.goldenYellow, size: 18),
                    ],
                  ),
                ),
              );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppTheme.creamDough,
      body: SafeArea(
        child: FutureBuilder<List<KitchenModel>>(
          future: _kitchensFuture,
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(
                child: CircularProgressIndicator(
                  valueColor: AlwaysStoppedAnimation<Color>(AppTheme.burntOrange),
                ),
              );
            }
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Kitchen Timeline',
                                style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w900, color: AppTheme.burntOrange)),
                            Text(
                      ...
                      IconButton(
                        icon: Container(
                          ...
                          child: const Icon(Icons.home_rounded, color: AppTheme.burntOrange, size: 20),
                        ),
                        ...
                        child: Column(
                          ...
                          children: [
                             Container(
                               ...
                               child: Image.asset(
                                 'assets/images/state_empty_history.jpeg',
                                 width: 160,
                                 height: 160,
                                 fit: BoxFit.contain,
                               ),
                            ...
                            Text(
                              'Start creating your first kitchen transformation!',
                              style: TextStyle(
                                  color: AppTheme.slate.withValues(alpha: 0.7),
                                  fontSize: 14),
                  ...
                  Expanded(
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        final kitchen = all[index];
                        final selected = _selectedIds.contains(kitchen.id);
                        ...
                        return _HistoryCard(
                          ...
                          onTap: () {
                            if (_isSelecting) {
                              setState(() {
                                if (selected) {
                                  _selectedIds.remove(kitchen.id);
                                } else {
                                  _selectedIds.add(kitchen.id);
                                }
                              });
                            } else {
                              _openDetail(context, kitchen, all);
                            }
                          },
                          onLongPress: () {
  ...
  void _openDetail(BuildContext ctx, KitchenModel kitchen, List<KitchenModel> all) {
    showModalBottomSheet(
      context: ctx,
      ...
      builder: (_) => HistoryDetailSheet(
        ...
        onDelete: () async {
          Navigator.pop(ctx);
          final confirm = await _showDeleteDialog(ctx, 'this kitchen');
          if (!confirm) return;
          final storage = context.read<StorageService>();
          final remaining = all.where((g) => g.id != kitchen.id).toList();
          await storage.saveKitchens(remaining);
          _reload();
        },
      ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          borderRadius: BorderRadius.circular(24),
          border: isSelected
              ? Border.all(color: AppTheme.burntOrange, width: 2.5)
              : null,
          boxShadow: [
            BoxShadow(
        ...
        child: ClipRRect(
          ...
          child: Stack(
            fit: StackFit.expand,
            children: [
              buildImage(kitchen.resultImagePath),
              // Gradient
              Positioned(
                bottom: 0,
                left: 0,
                ...
                Positioned(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(
                        color: isSelected ? AppTheme.burntOrange : Colors.white54,
                        width: 2,
                      ),
                    ),
                    child: isSelected
...
class _HistoryDetailSheetState extends State<HistoryDetailSheet> {
              ...
              child: Stack(
                ...
                children: [
                  ...
                  Positioned(
                    top: 12, right: 12,
                    child: _SliderLabel(
                        text: 'AFTER', color: AppTheme.butterYellow),
                  ),
                ],
              ),
            ),
  ...
  SnackBar _snackBar(String msg, {bool isSuccess = false}) {
    return SnackBar(
      ...
      backgroundColor: isSuccess ? AppTheme.burntOrange : AppTheme.slate,
      content: Text(msg, style: const TextStyle(fontWeight: FontWeight.w500)),
    );
  }

  void _openFullscreen(BuildContext ctx, KitchenModel g) {
    Navigator.push(
      ctx,
      MaterialPageRoute(
...
class _FullscreenView extends StatefulWidget {
  ...
  const _FullscreenView({required this.kitchen, required this.buildImage});
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: Stack(
          fit: StackFit.expand,
          children: [
            widget.buildImage(widget.kitchen.originalImagePath),
            ClipRect(
              clipper: _RightClipper(_sliderX),
              child: widget.buildImage(widget.kitchen.resultImagePath),
            ),
            // Divider
            Positioned(
              left: MediaQuery.of(context).size.width * _sliderX - 20,
            ...
            Positioned(
              top: 60, right: 16,
              child: _SliderLabel(text: 'AFTER', color: AppTheme.butterYellow),
            ),
            // Close
            Positioned(
              top: 52,
      ...
      child: Stack(
        ...
        children: [
          Container(
            ...
            child: Icon(icon, color: AppTheme.burntOrange, size: 22),
          ),
          if (badge != null)
            Positioned(
              top: -3,
