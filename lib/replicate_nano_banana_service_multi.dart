...
class ReplicateKitchenAIService {
  ...
  /// Builds an optimized prompt for kitchen image-to-image generation.
  ///
  /// The prompt is constructed to:
  /// 1. Explicitly preserve kitchen structure, perspective, and dimensions
  /// 2. Only modify the specified kitchen materials
  /// 3. Maintain lighting consistency with the original kitchen photo
  /// 4. Keep the same camera angle and composition
  static String buildKitchenPrompt({
    ...
    parts.add(
      'Transform this existing kitchen photo while preserving the exact same '
      'interior structure, dimensions, camera angle, perspective, boundary positions, '
      'and spatial layout.',
    );

    // ── Style ──
    final style = config['style'] as String? ?? 'Modern';
    parts.add('Apply a $style kitchen design style.');
    ...
    if (layoutConfig != null) {
      ...
      if (hasIsland) {
        parts.add(
          'Include a decorative kitchen island feature if space allows.',
        );
      }
      if (hasBreakfastBar) {
  ...
  /// Generates a kitchen redesign image using image-to-image transformation.
  ///
  /// [images] - Source kitchen photo(s) as byte arrays.
    ...
    final body = {
      ...
      'input': {
        ...
        'negative_prompt':
            ...
            'different layout, distorted kitchen, warped kitchen, '
            'different cabinet positions, different floor size, '
